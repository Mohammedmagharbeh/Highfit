...
export default function JobsPage() {
  ...
  const applyJob = async () => {
    ...
    try {
      await axios.post(
        `${import.meta.env.VITE_deploy ? "api" : import.meta.env.VITE_BASE_URL}/apply`,
        formData,
        {
  ...
  return (
    <div
      className="min-h-screen bg-[#0a0a0a] text-white font-sans pb-20 pt-28"
      dir={isAr ? "rtl" : "ltr"}
    >
      {/* Hero Header */}
      <div className="relative py-16 sm:py-24 overflow-hidden">
        <div
          className={`absolute inset-0 bg-orange-500/5 ${isAr ? "-skew-y-3 origin-right" : "skew-y-3 origin-left"}`}
        ></div>
                ...
                >
                  <div className="flex justify-between items-start">
                    <div className={isAr ? "text-right" : "text-left"}>
                      <h3
                        className={`text-xl font-bold mb-1 ${form.jobId === job._id ? "text-white" : "text-orange-500"}`}
                      >
        ...
        {form.jobId && (
          <section className="bg-[#111] border border-white/10 rounded-3xl overflow-hidden shadow-2xl animate-in fade-in slide-in-from-bottom-8">
            <div className="bg-orange-500 p-6 text-center">
              <h3 className="text-xl font-bold uppercase italic tracking-widest">
                {t("apply_for_job")}
              </h3>
            </div>
            ...
            <div className="p-8 sm:p-12">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div className="sm:col-span-2 text-right">
                  <label
                    className={`text-xs font-bold text-white/40 uppercase mb-2 block ${isAr ? "text-right" : "text-left"}`}
                  >
                    {t("start_date")}
                  </label>
                  <div className="relative">
                    <Calendar
                      className={`absolute ${isAr ? "right-4" : "left-4"} top-1/2 -translate-y-1/2 text-orange-500`}
                      size={18}
                    />
                    <input
                      type="date"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-8">
                ...
                <FileUploader
                  isAr={isAr}
                  label={t("experience_certificate")}
              ...
              <div className="mt-10 p-6 bg-white/5 rounded-2xl border border-white/10">
                <p
                  className={`font-bold mb-4 flex items-center gap-2 ${isAr ? "flex-row" : "flex-row"}`}
                >
                  ...
                  {["yes", "no"].map((opt) => (
                    ...
                    >
                      <input
                        ...
                        name="workedBefore"
                        value={opt}
                        checked={form.workedBefore === opt}
                        onChange={() => setForm({ ...form, workedBefore: opt })}
                        className="hidden"
                      />
                      <div
                        className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${form.workedBefore === opt ? "border-orange-500 bg-orange-500" : "border-white/20"}`}
                      >
                        {form.workedBefore === opt && (
                ...
                {form.workedBefore === "yes" && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6 animate-in zoom-in-95">
                    <input
                      ...
                      className={`bg-[#1a1a1a] border border-white/10 rounded-xl p-4 outline-none focus:border-orange-500 ${isAr ? "text-right" : "text-left"}`}
                    />
                    <input
...
function InputField({ icon, label, type = "text", value, onChange, isAr }) {
  return (
    <div className="relative group">
      <label
        className={`text-xs font-bold text-white/40 uppercase mb-2 block px-1 group-focus-within:text-orange-500 transition-colors ${isAr ? "text-right" : "text-left"}`}
      >
        {label}
      </label>
      <div className="relative">
  ...
  return (
    <div className="group">
      ...
      <div className="relative border-2 border-dashed border-white/10 rounded-2xl p-4 hover:border-orange-500/50 hover:bg-orange-500/5 transition-all">
        ...
        />
        <div
          className={`flex items-center gap-3 text-white/60 group-hover:text-white transition-colors ${isAr ? "flex-row-reverse" : ""}`}
        >
