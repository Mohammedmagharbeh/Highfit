...
const BASE_URL = import.meta.env.VITE_deploy ? "api" : import.meta.env.VITE_BASE_URL;
    ...
    <div className="bg-black min-h-screen py-20 px-6 font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
        {subscriptions.map((sub) => {
          const title = i18n.language.startsWith("ar")
            ? sub.title.ar
            : sub.title.en;
...
// ------------------ Modal Component (Fixed Phone Fetching) ------------------
const SubscriptionModal = ({ sub, onClose, i18n }) => {
  const { t } = useTranslation();
  const [fullName, setFullName] = useState("");
  const [phoneNumber, setPhoneNumber] = useState(""); // سيبقى فارغاً حتى يكتمل التحميل
  ...
  useEffect(() => {
    const getUserData = async () => {
      ...
      try {
        ...
        // --- تعديل ذكي لجلب الرقم مهما كان شكل الـ Object الراجع ---
        ...
        if (phone) {
          ...
          console.error("Phone not found in API response:", res.data);
          toast.error("لم نتمكن من العثور على رقم هاتفك المسجل");
        }
      } catch (err) {
        ...
        {fetchingUser ? (
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="text-[10px] font-black text-white/30 uppercase mb-1 block text-right">
                  {t("modal.phone")}
                </label>
                {/* تم إضافة defaultValue والتأكد من القيمة */}
                <input
                  readOnly
                  type="text"
                  value={phoneNumber || ""}
                  placeholder="رقم الهاتف غير متوفر"
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-white/10 border border-white/10 p-4 rounded-xl text-white outline-none focus:border-orange-500 font-bold text-right text-sm"
            ...
            <div>
              ...
              <select
                className="w-full bg-white/10 border border-white/10 p-4 rounded-xl text-white outline-none font-bold text-right text-sm"
            ...
            <div className="pt-6 border-t border-white/5 mt-4 flex flex-row-reverse justify-between items-center px-2">
              <div className="text-right">
                <span className="text-white/20 font-bold text-[10px] uppercase italic block tracking-tighter">
                  {t("modal.price")}
                </span>
                <span className="text-3xl font-black text-white italic">
                  {selectedPlan.price}{" "}
                  <small className="text-[11px] text-orange-500 uppercase">
                    JOD
                  </small>
                </span>
              </div>
