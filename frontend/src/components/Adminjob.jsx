...
export default function AdminJobs() {
  ...
  const viewApplications = async (jobId) => {
    ...
    setApplications([]);
    try {
      const res = await axios.get(`api/jobs/applications/${jobId}`);
      setApplications(res.data);
      if (res.data.length > 0) {
        toast.success(
          isAr
            ? `تم العثور على ${res.data.length} متقدمين`
            : `Found ${res.data.length} applicants`,
        );
      }
    } catch (err) {
  ...
  const addJob = async () => {
    ...
    try {
      ...
      toast.error(isAr ? "فشل في إضافة الوظيفة" : "Failed to add job", {
        id: loadingToast,
      });
    }
  };
  ...
  return (
    <div
      className="min-h-screen bg-[#0a0a0a] text-white font-sans pb-20 pt-10"
      dir={isAr ? "rtl" : "ltr"}
    >
      {/* Header */}
        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-black italic tracking-tighter">
              <span className="text-orange-500">HIGH</span> FIT{" "}
              {isAr ? "الإدارة" : "ADMIN"}
            </h1>
          </div>
          <button
      ...
      <main className="max-w-7xl mx-auto px-6 mt-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12 text-center md:text-start">
          <div className="bg-white/5 border border-white/10 p-8 rounded-[2rem]">
            <p className="text-white/40 text-xs font-bold uppercase tracking-[0.2em] mb-2">
              {isAr ? "الوظائف النشطة" : "Active Jobs"}
            </p>
            <p className="text-5xl font-black italic">{jobs.length}</p>
          </div>
          <div className="bg-white/5 border border-white/10 p-8 rounded-[2rem]">
            <p className="text-white/40 text-xs font-bold uppercase tracking-[0.2em] mb-2">
              {isAr ? "المتقدمين الذين تم العثور عليهم" : "Applicants Found"}
            </p>
          ...
          <div className="bg-[#111] border border-orange-500/20 rounded-[2.5rem] p-8 mb-12 animate-in fade-in slide-in-from-top-4">
            <div className="grid md:grid-cols-2 gap-6">
              <input
                ...
                className={`bg-white/5 border border-white/10 p-4 rounded-xl outline-none focus:border-orange-500 ${isAr ? "text-left" : ""}`}
              />
              <input
                placeholder="المسمى الوظيفي (AR)"
                dir="rtl"
                value={form.titleAr}
                onChange={(e) => setForm({ ...form, titleAr: e.target.value })}
                className="bg-white/5 border border-white/10 p-4 rounded-xl outline-none focus:border-orange-500 text-right"
              />
              <input
                placeholder="Type (EN)"
                value={form.typeEn}
                onChange={(e) => setForm({ ...form, typeEn: e.target.value })}
                className={`bg-white/5 border border-white/10 p-4 rounded-xl outline-none focus:border-orange-500 ${isAr ? "text-left" : ""}`}
              />
              <input
                placeholder="نوع الدوام (AR)"
                dir="rtl"
                value={form.typeAr}
            ...
            <button
              onClick={addJob}
              className="w-full bg-orange-500 mt-6 py-4 rounded-xl font-black uppercase italic hover:bg-orange-600 transition-all"
            >
        ...
        <div className="relative mb-10">
          <Search
            className={`absolute ${isAr ? "right-5" : "left-5"} top-1/2 -translate-y-1/2 text-white/20`}
            size={20}
          />
          <input
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-20">
          {filteredJobs.map((job) => (
            <div
              key={job._id}
              className={`p-8 rounded-[2rem] border transition-all group ${selectedJob === job._id ? "border-orange-500 bg-orange-500/5 shadow-2xl" : "border-white/5 bg-white/5"}`}
            >
          ...
          <div className="animate-in slide-in-from-bottom-10 pt-10 border-t border-white/10">
            ...
            {loadingApps ? (
              ...
              <div className="py-20 text-center bg-white/5 rounded-3xl border border-dashed border-white/10 text-white/20 italic">
                {isAr
                  ? "لا يوجد متقدمين لهذه الوظيفة بعد."
              ...
              <div className="space-y-6">
                {applications.map((app) => (
                  <div
                    key={app._id}
                    className="bg-[#111] border border-white/5 rounded-[2rem] p-8 md:p-10 flex flex-col gap-8"
                  >
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                      <div>
                        <p className="text-[10px] text-orange-500 font-black mb-1 uppercase">
                      ...
                      <div>
                        ...
                        <p className="font-bold text-base tracking-tighter">
                          {app.phone}
                        </p>
                      </div>
                      <div>
                        ...
                        <p className="font-bold text-base">{app.nationality}</p>
                      </div>
                      <div>
                        <p className="text-[10px] text-orange-500 font-black mb-1 uppercase">
                          {isAr ? "العمر" : "Age"}
                    ...
                    <div className="bg-white/5 border border-white/10 p-6 rounded-2xl">
                      <div
                        className={`flex flex-col md:flex-row gap-6 md:items-center ${isAr ? "md:flex-row" : ""}`}
                      >
                        ...
                        {app.workedBefore === "yes" && (
                          <div
                            className={`flex flex-col md:flex-row gap-6 md:gap-12 border-t md:border-t-0 ${isAr ? "md:border-r md:pr-8" : "md:border-l md:pl-8"} border-white/10 pt-4 md:pt-0 w-full`}
                          >
                            <div className="flex items-center gap-3">
                              <Building2 size={16} className="text-white/40" />
                              <div>
                                <p className="text-[9px] text-white/40 font-bold uppercase">
                                  {isAr ? "آخر شركة" : "Last Company"}
                                </p>
                                <p className="font-bold italic text-sm">
                                  {app.previousJobs || "N/A"}
                                </p>
                              </div>
                            </div>
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p className="text-[9px] text-white/40 font-bold uppercase">
                                  {isAr ? "آخر مسمى" : "Last Title"}
                                </p>
                                <p className="font-bold italic text-sm">
                                  {app.previousTitle || "N/A"}
                                </p>
                              </div>
                            </div>
                          </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-4 pt-4 border-t border-white/5">
                      {app.resume && app.resume !== "null" && (
                        <a
                          href={app.resume}
                          target="_blank"
                          className="flex-1 bg-white text-black hover:bg-orange-500 hover:text-white py-4 rounded-xl font-black text-[10px] tracking-widest flex items-center justify-center gap-2 transition-all shadow-xl shadow-black/20"
                        >
                          <FileText size={16} />{" "}
                          {isAr ? "عرض السيرة الذاتية" : "VIEW RESUME"}
                        </a>
                      )}
                      {app.experienceCertificate &&
                        app.experienceCertificate !== "null" && (
                          <a
                            href={app.experienceCertificate}
                            target="_blank"
                            className="flex-1 bg-white/5 border border-white/10 hover:border-orange-500 py-4 rounded-xl font-black text-[10px] tracking-widest flex items-center justify-center gap-2 transition-all"
                          >
                            <Award size={16} />{" "}
                            {isAr ? "شهادة الخبرة" : "EXPERIENCE CERT"}
                          </a>
                        )}
                    </div>
                  </div>
