...
const DefaultTemplates = () => {
  ...
  const allIds = Array.from(
    new Set([...Object.keys(trainingPlans), ...Object.keys(nutritionPlans)]),
  );
  ...
  return (
    ...
    >
      <div
        className={`absolute top-24 z-50 flex gap-3 ${isRtl ? "right-4 md:right-8" : "left-4 md:left-8"}`}
      >
        {isEditMode ? (
          <>
            <button
          ...
          <div className="w-full mb-8">
            <div className="w-full bg-neutral-900 p-4 rounded-2xl flex flex-wrap gap-4 items-center justify-center border border-neutral-800">
              {allIds.map((id) => {
                ...
                const name =
                  trainingPlans[id]?.arabicTitle ||
                  nutritionPlans[id]?.title ||
                  id;
                    ...
                    {isEditMode && (
                      <button
                        onClick={(e) => {
                          ...
                          if (
                            window.confirm(
                              t("templates.actions.confirm_delete"),
                            )
                          ) {
                    ...
                    {isEditMode ? (
                      <>
                        <input
                          ...
                          placeholder={t(
                            "templates.training.placeholder_title",
                          )}
                        />
                        <textarea
                      ...
                      <>
                        <h2 className="text-2xl font-bold mb-2 text-white">
                          {activeTraining?.arabicTitle ||
                            t("templates.training.default_title")}
                        </h2>
                    ...
                    {isEditMode ? (
                      <>
                        <input
                          ...
                          className="text-2xl font-bold mb-2 bg-transparent border-b-2 border-dashed border-orange-500/50 px-2 py-1 focus:border-orange-500 outline-none w-full text-white"
                          placeholder={t(
                            "templates.nutrition.placeholder_title",
                          )}
                        />
                        <textarea
                          ...
                          className="text-neutral-400 w-full bg-transparent border-b-2 border-dashed border-neutral-700 px-2 py-1 mt-2 focus:border-orange-500 outline-none resize-none"
                          placeholder={t(
                            "templates.nutrition.placeholder_desc",
                          )}
                          rows={2}
                        />
                      </>
                      ...
                      <>
                        <h2 className="text-2xl font-bold mb-2 text-white">
                          {activeNutrition?.title ||
                            t("templates.nutrition.default_title")}
                        </h2>
                        <p className="text-neutral-400">
                  ...
                  <div className="space-y-6">
                    ...
                    {isEditMode && (
                      ...
                      >
                        <span className="text-4xl">+</span>
                        <span className="font-bold text-lg">
                          {t("templates.nutrition.add_meal")}
                        </span>
                      </button>
                    )}
                  </div>
