...
const DefaultTemplates = () => {
  ...
  const trainingPlans = plansHook.data;
  const nutritionPlans = nutritionHook.data;
  const allIds = Array.from(new Set([...Object.keys(trainingPlans), ...Object.keys(nutritionPlans)]));

  if (plansHook.loading || nutritionHook.loading) {
    return (
      <div className="min-h-screen bg-neutral-950 text-white flex items-center justify-center p-8 text-xl animate-pulse">
        {t("templates.loading")}
      </div>
    );
  }

  let safePlanId = activePlanId || (allIds.length > 0 ? allIds[0] : "");

  const activeTraining = trainingPlans[safePlanId] || { training: [] };
  const activeNutrition = nutritionPlans[safePlanId] || { meals: [] };

  const handleCreateNewTemplate = () => {
    const newId = prompt(t("templates.prompts.new_id"));
    if (!newId) return;

    if (allIds.includes(newId)) {
      alert(t("templates.errors.exists"));
      return;
    }

    const title = prompt(t("templates.prompts.new_title"));
    if (!title) return;

    plansHook.updateTrainingInfo(newId, "title", newId);
    plansHook.updateTrainingInfo(newId, "arabicTitle", title);
    plansHook.updateTrainingInfo(newId, "planId", newId);

    nutritionHook.updateNutritionInfo(newId, "title", title);
    nutritionHook.updateNutritionInfo(newId, "programId", newId);
  ...
  return (
    <div className={`min-h-screen bg-neutral-950 text-neutral-100 font-sans p-4 md:p-8`} dir={isRtl ? "rtl" : "ltr"}>
      <div className={`absolute top-24 z-50 flex gap-3 ${isRtl ? "right-4 md:right-8" : "left-4 md:left-8"}`}>
        {isEditMode ? (
          <>
            <button
              onClick={() => {
            ...
            >
              <Save className="w-4 h-4" />
              {plansHook.isSaving || nutritionHook.isSaving ? t("training.actions.saving") : t("training.actions.save")}
            </button>
          </>
        ) : (
          canEdit && (
            <button
              ...
              className="flex items-center gap-2 bg-neutral-800 hover:bg-neutral-700 text-white px-4 py-2 rounded-xl transition-colors shadow-lg border border-neutral-700 font-bold"
            >
              <Edit2 className="w-4 h-4" /> {t("templates.actions.edit")}
            </button>
          )
        )}
      </div>
      ...
      <div className="max-w-6xl mx-auto flex flex-col items-center">
        {allIds.length > 0 && (
          <div className="w-full mb-8">
            <div className="w-full bg-neutral-900 p-4 rounded-2xl flex flex-wrap gap-4 items-center justify-center border border-neutral-800">
              {allIds.map((id) => {
                const isActive = safePlanId === id;
                const name = trainingPlans[id]?.arabicTitle || nutritionPlans[id]?.title || id;
                  ...
                  <div key={id} className="relative">
                    ...
                    {isEditMode && (
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          if (window.confirm(t("templates.actions.confirm_delete"))) {
                            plansHook.deleteTemplate(id);
                            nutritionHook.deleteTemplate(id);
                            if (isActive) setActivePlanId("");
                          }
                        }}
                        className={`absolute -top-2 bg-red-600 hover:bg-red-500 text-white p-2 rounded-full shadow-lg z-10 transition-transform hover:scale-110 ${isRtl ? "-left-2" : "-right-2"}`}
                      >
                        <Trash2 className="w-4 h-4" />
                      </button>
                    )}
              ...
              {isEditMode && (
                ...
                >
                  <PlusCircle className="w-6 h-6" />
                  <span>{t("templates.actions.add_new")}</span>
                </button>
              )}
            </div>
          </div>
        ...
        {safePlanId && (
          <div className="w-full bg-neutral-900 rounded-3xl border border-neutral-800 overflow-hidden shadow-2xl">
            <DashboardTabs activeTab={activeSubTab} onTabChange={setActiveSubTab} />
            ...
            <div className="p-6 md:p-8 min-h-[50vh]">
              {activeSubTab === "training" && (
                <div className="space-y-8 animate-in fade-in duration-500">
                    ...
                    {isEditMode ? (
                      <>
                        <input
                          ...
                          className="text-2xl font-bold mb-2 bg-transparent border-b-2 border-dashed border-orange-500/50 px-2 py-1 focus:border-orange-500 outline-none w-full text-white"
                          placeholder={t("templates.training.placeholder_title")}
                        />
                        <textarea
                          value={activeTraining?.desc || ""}
                          onChange={(e) => plansHook.updateTrainingInfo(safePlanId, "desc", e.target.value)}
                          className="text-neutral-400 w-full bg-transparent border-b-2 border-dashed border-neutral-700 px-2 py-1 mt-2 focus:border-orange-500 outline-none resize-none"
                          placeholder={t("templates.training.placeholder_desc")}
                          rows={2}
                        />
                      </>
                    ) : (
                      <>
                        <h2 className="text-2xl font-bold mb-2 text-white">
                          {activeTraining?.arabicTitle || t("templates.training.default_title")}
                        </h2>
                  ...
                  <div className="grid gap-6 items-stretch">
                    {activeTraining?.training?.map((day, dIdx) => (
                      <TrainingDayCard key={dIdx} day={day} dayIndex={dIdx} planId={safePlanId} isEditMode={isEditMode} plansHook={plansHook} />
                    ))}
                    {isEditMode && (
                      <button
                        onClick={() => plansHook.addDay(safePlanId)}
                      ...
                      >
                        <span className="text-4xl">+</span>
                        <span className="font-bold text-lg">{t("templates.training.add_day")}</span>
                      </button>
                    )}
                  </div>
                </div>
              ...
              {activeSubTab === "nutrition" && (
                <div className="space-y-8 animate-in fade-in duration-500">
                    ...
                    {isEditMode ? (
                      <>
                        <input
                          ...
                          className="text-2xl font-bold mb-2 bg-transparent border-b-2 border-dashed border-orange-500/50 px-2 py-1 focus:border-orange-500 outline-none w-full text-white"
                          placeholder={t("templates.nutrition.placeholder_title")}
                        />
                        <textarea
                          value={activeNutrition?.desc || ""}
                          onChange={(e) => nutritionHook.updateNutritionInfo(safePlanId, "desc", e.target.value)}
                          className="text-neutral-400 w-full bg-transparent border-b-2 border-dashed border-neutral-700 px-2 py-1 mt-2 focus:border-orange-500 outline-none resize-none"
                          placeholder={t("templates.nutrition.placeholder_desc")}
                          rows={2}
                        />
                      </>
                    ) : (
                      <>
                        <h2 className="text-2xl font-bold mb-2 text-white">
                          {activeNutrition?.title || t("templates.nutrition.default_title")}
                        </h2>
                  ...
                  <div className="space-y-6">
                    {activeNutrition?.meals?.map((meal, mIdx) => (
                      <NutritionMealCard key={mIdx} meal={meal} mealIndex={mIdx} planId={safePlanId} isEditMode={isEditMode} nutritionHook={nutritionHook} />
                    ))}
                    {isEditMode && (
                      <button
                        onClick={() => nutritionHook.addMeal(safePlanId)}
                      ...
                      >
                        <span className="text-4xl">+</span>
                        <span className="font-bold text-lg">{t("templates.nutrition.add_meal")}</span>
                      </button>
                    )}
                  </div>
                </div>
