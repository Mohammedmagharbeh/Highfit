...
//       if (response.status === 200) {
// // التخزين في السشن (ينتهي بإغلاق المتصفح) بدل اللوكل ستورج
...
import { useCart } from "../context/CartContext"; // استيراد الـ CartContext
...
export default function LoginPage() {
  ...
  // مناداة دالة fetchCart عشان نحدث السلة فوراً بعد اللوجن
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        // 1. تخزين التوكن في السشن
        sessionStorage.setItem("token", response.data.token);
        
        // 2. تخزين بيانات اليوزر كاملة (ضروري جداً للسلة والـ Refresh)
        sessionStorage.setItem("user", JSON.stringify(response.data.user)); 
        
        // 3. تحديث السلة فوراً قبل الانتقال للهوم
