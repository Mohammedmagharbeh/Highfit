...
const UserOrder = () => {
  ...
  const handleAddToCartClick = async () => {
    ...
    try {
      ...
      toast.success(`${t("added")} ${mealName} ${t("to_cart_success")} 🛒`, {
        id: loadingToast,
      });
  ...
  return (
    <div
      className="min-h-screen bg-[#050505] text-white font-sans pb-28"
      dir={isAr ? "rtl" : "ltr"}
    >
      {/* Header */}
            ...
            <div className="relative w-24 h-24 bg-white/5 rounded-2xl flex-shrink-0 border border-white/5 overflow-hidden">
              {meal.image ? (
                <img
                  ...
                  alt={meal.name[i18n.language]}
                />
              ) : (
                <div className="w-full h-full flex items-center justify-center text-white/10 italic text-[10px]">
                  {t("no_image")}
                </div>
              )}
              <div
                className={`absolute bottom-1 ${isAr ? "left-1" : "right-1"} bg-orange-500 text-black p-1 rounded-lg`}
              >
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-50 flex items-end">
          <div className="bg-[#0c0c0c] w-full rounded-t-[3rem] p-8 border-t border-white/10 animate-in slide-in-from-bottom duration-500 relative max-h-[85vh] overflow-y-auto">
            <button
              ...
              className={`absolute top-6 ${isAr ? "left-6" : "right-6"} text-gray-500 hover:text-white`}
            >
              <X size={28} />
            </button>
            ...
            <div className="mb-8 pt-4 text-center flex flex-col items-center">
              {selectedMeal.image && (
                <img
                  ...
                  className="w-28 h-28 rounded-3xl object-cover mb-4 border border-white/10"
                  alt=""
                />
              )}
              <h3 className="text-2xl font-black">
                {selectedMeal.name[i18n.language] || selectedMeal.name.en}
              </h3>
              <div className="text-gray-400 mt-1">
                {selectedMeal.price} {t("currency")}
              </div>
            </div>
            ...
            <div className="space-y-6 mb-8 max-w-md mx-auto">
              ...
              <div
                className={`relative group ${isAr ? "text-right" : "text-left"}`}
              >
                <MessageSquare
                  className={`absolute ${isAr ? "right-4" : "left-4"} top-4 text-gray-600 group-focus-within:text-orange-500`}
                  size={18}
                />
                <textarea
            ...
            >
              {t("add_to_cart_btn")} •{" "}
              {(selectedMeal.price * orderData.quantity).toFixed(2)}{" "}
              {t("currency")}
            </button>
          </div>
        </div>
