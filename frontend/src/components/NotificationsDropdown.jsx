...
const NotificationsDropdown = () => {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      const res = await axios.get(
        `${import.meta.env.deploy ? "api" : import.meta.env.VITE_BASE_URL}/notifications`,
        {
  ...
  useEffect(() => {
    ...
    const handleNewNotification = (notification) => {
      ...
      if (isForMe) {
        ...
        toast.custom((t) => (
          <div
            className={`${t.visible ? "animate-enter" : "animate-leave"} max-w-md w-full bg-[#111] border border-orange-500 shadow-lg rounded-xl pointer-events-auto flex ring-1 ring-black ring-opacity-5`}
            dir="rtl"
          >
  ...
  const markAsRead = async (id) => {
    try {
      if (!token) return;
      await axios.put(
        `${import.meta.env.deploy ? "api" : import.meta.env.VITE_BASE_URL}/notifications/${id}/read`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const markAllAsRead = async () => {
    try {
      if (!token) return;
      await axios.put(
        `${import.meta.env.deploy ? "api" : import.meta.env.VITE_BASE_URL}/notifications/read-all`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        },
    ...
    <div className="relative notifications-container">
      ...
      {isOpen && (
        <div className="absolute left-1/2 -translate-x-1/2 md:left-auto md:translate-x-0 md:right-0 mt-2 w-80 bg-[#0a0a0a] rounded-2xl shadow-2xl border border-white/10 z-[1000] overflow-hidden">
          <div
            className="p-4 border-b border-white/10 flex justify-between items-center bg-[#111]"
            dir="rtl"
          >
          ...
          <div className="max-h-96 overflow-y-auto w-full custom-scrollbar">
            {notifications.length === 0 ? (
              ...
              notifications.map((notif) => (
                ...
                >
                  <div
                    className={`mt-1 flex-shrink-0 ${!notif.isRead ? "text-orange-500" : "text-gray-500"}`}
                  >
                    <Bell size={18} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4
                      className={`text-sm truncate ${!notif.isRead ? "font-bold text-white" : "text-gray-400"}`}
                    >
                      {notif.title}
                    </h4>
