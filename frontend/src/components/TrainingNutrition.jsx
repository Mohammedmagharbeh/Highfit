  ...
  return (
    <div className="relative pt-6">
      {(isAdminOrLead || isCoach) && (
        <div className="flex justify-center mb-6 relative z-50">
          <div 
            className={`bg-neutral-900 border border-neutral-800 rounded-2xl p-1 flex shadow-lg ${isRtl ? 'flex-row-reverse' : 'flex-row'}`}
            dir={isRtl ? "rtl" : "ltr"}
          >
            <button
              onClick={() => setActiveMainTab("users")}
            ...
            >
              {t("training.tabs.default_templates")}
            </button>
          </div>
        </div>
      )}
      ...
      {activeMainTab === "users" ? (
        <div className="relative">
          {/* تم تعديل مكان الأزرار لتتناسب مع اللغة */}
          <div className={`absolute top-4 z-50 flex gap-3 ${isRtl ? 'right-4 md:right-8' : 'left-4 md:left-8'}`}>
            {isEditMode ? (
              <div className="flex gap-3" dir={isRtl ? "rtl" : "ltr"}>
                ...
                >
                  <Save className="w-4 h-4" />
                  {programHook.isSaving ? t("training.actions.saving") : t("training.actions.save")}
                </button>
              </div>
            ) : (
              isAdminOrLead && (
                ...
                >
                  ...
                  {t("training.actions.edit_manual")}
                </button>
              )
            )}
          </div>
          ...
          <TrainingDashboard
            ...
            description={t("training.dashboard.description")}
            isEditMode={isEditMode || false}
            programHook={programHook}
            trainingTemplates={plansHook.data}
            nutritionTemplates={nutritionHook.data}
