...
const Checkout = () => {
  ...
  useEffect(() => {
    // جلب المناطق إذا كان الطلب توصيل والتوكن موجود (أي نوع توكن)
    if (orderType === "delivery" && token) {
      const fetchLocations = async () => {
        try {
          const res = await axios.get(
            `${import.meta.env.deploy ? "api" : import.meta.env.VITE_BASE_URL}/locations/get`,
            { headers: { Authorization: `Bearer ${token}` } },
          );
          setLocations(res.data.locations || []);
        } catch (error) {
          console.error("Fetch locations error", error);
          // لا نظهر الخطأ هنا إلا إذا فشل الطلب تماماً
        }
      };
      fetchLocations();
    }
  ...
  const handlePlaceOrder = async (e) => {
    ...
    try {
      const deliveryFee =
        orderType === "delivery" ? selectedArea?.deliveryCost || 0 : 0;
      ...
      const orderPayload = {
        ...
        items: cart.products.map((p) => ({
          ...
          mealName:
            p.productId?.name[i18n.language] || p.productId?.name?.en || "Meal",
      ...
      if (response.status === 201 || response.status === 200) {
        ...
        await clearCart();
        // التوجيه لصفحة الطلبات لمتابعة الحالة
      ...
      <div className="max-w-2xl mx-auto space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button
              onClick={() => navigate(-1)}
              className="p-3 bg-white/5 rounded-2xl text-gray-400 hover:text-white border border-white/5 transition-all"
            >
        ...
        <div className="grid grid-cols-2 gap-4">
          <button
            ...
            className={`p-6 rounded-[2rem] border transition-all duration-300 flex flex-col items-center gap-3 ${orderType === "delivery" ? "border-orange-500 bg-orange-500/10 text-orange-500 shadow-xl shadow-orange-500/5" : "border-white/5 bg-[#0a0a0a] text-gray-600"}`}
          >
            <Truck size={32} />
          ...
          <button
            ...
            className={`p-6 rounded-[2rem] border transition-all duration-300 flex flex-col items-center gap-3 ${orderType === "pickup" ? "border-orange-500 bg-orange-500/10 text-orange-500 shadow-xl shadow-orange-500/5" : "border-white/5 bg-[#0a0a0a] text-gray-600"}`}
          >
            <Store size={32} />
            <span className="font-black uppercase text-xs tracking-widest">
              {t("pickup")}
        ...
        <form onSubmit={handlePlaceOrder} className="space-y-6">
          <div className="bg-[#0a0a0a] border border-white/5 rounded-[2.5rem] p-8 space-y-8 shadow-2xl relative overflow-hidden">
            <h3
              className={`font-black italic uppercase flex items-center gap-2 text-gray-400 tracking-tighter text-sm ${isAr ? "text-right" : "text-left"}`}
            >
            ...
            <div className="space-y-5">
              <div className="relative group">
                <User
                  className={`absolute ${isAr ? "left-4" : "right-4"} top-1/2 -translate-y-1/2 text-gray-600 group-focus-within:text-orange-500 transition-colors`}
                  size={20}
                />
                <input
                  required
                  className={`w-full bg-white/5 border border-white/10 p-5 ${isAr ? "pl-12 text-right" : "pr-12 text-left"} rounded-2xl outline-none focus:border-orange-500/50 transition-all`}
              ...
              {orderType === "delivery" && (
                <div className="space-y-5 animate-in fade-in slide-in-from-top-4 duration-500">
                  <div className="relative group">
                    <MapPin
                      className={`absolute ${isAr ? "left-4" : "right-4"} top-1/2 -translate-y-1/2 text-gray-600 group-focus-within:text-orange-500 transition-colors z-10`}
                      size={20}
                    />
                    <select
                    ...
                    <ChevronDown
                      size={18}
                      className={`absolute ${isAr ? "right-4" : "left-4"} top-1/2 -translate-y-1/2 pointer-events-none text-gray-600`}
                    />
                  </div>
                </div>
              )}
              ...
              <div className="relative group">
                ...
                <input
                  required
                  type="tel"
                  className={`w-full bg-white/5 border border-white/10 p-5 ${isAr ? "pl-12 text-left" : "pr-12 text-right"} rounded-2xl outline-none focus:border-orange-500/50 transition-all font-mono`}
              ...
              {orderType === "delivery" && selectedArea && (
                <div className="flex justify-between text-sm text-gray-500 italic">
                  ...
                  <span className="text-orange-500/80">
                    +{selectedArea.deliveryCost.toFixed(2)} {t("currency")}
                  </span>
                </div>
              )}
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-orange-500 hover:bg-orange-600 text-[#050505] font-black py-7 rounded-[2.5rem] transition-all flex items-center justify-center gap-4 text-2xl shadow-2xl disabled:opacity-50"
          >
