...
const Checkout = () => {
  ...
  const sessionData = JSON.parse(sessionStorage.getItem("user") || sessionStorage.getItem("staffUser"));
  const token = sessionStorage.getItem("token") || sessionStorage.getItem("staffToken");

  const [userName, setUserName] = useState(sessionData?.name || sessionData?.username || "");
  const [addressNote, setAddressNote] = useState("");
  const [phone, setPhone] = useState(sessionData?.phone || "");
