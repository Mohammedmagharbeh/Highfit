...
exports.placeOrder = async (req, res) => {
  try {
    ...
    const { items, userName, userPhone, address, totalAmount, notes } =
      req.body;
    ...
    res
      .status(201)
      .json({ message: "تم إرسال طلبك بنجاح للشيف 🚀", order: newOrder });
  } catch (error) {
    // ملاحظة: شلنا كلمة next عشان نتفادى خطأ "next is not a function"
    res
      .status(400)
      .json({ message: "فشل في عملية الطلب", error: error.message });
  }
};
...
exports.confirmOrder = async (req, res) => {
  try {
    ...
    if (!phone) {
      return res
        .status(400)
        .json({ message: "رقم الهاتف مطلوب لإرسال التأكيد" });
    }
    ...
    res
      .status(500)
      .json({ message: "فشل إرسال رسالة التأكيد", error: error.message });
  }
};
