...
exports.subscribe = async (req, res) => {
  try {
    ...
    if (
      durationText.includes("سنة") ||
      durationText.includes("عام") ||
      durationText.includes("year")
    ) {
      daysToAdd = 365;
    } else if (
      durationText.includes("شهرين") ||
      durationText.includes("2 month")
    ) {
      daysToAdd = 60;
    } else if (
      durationText.includes("أسبوع") ||
      durationText.includes("اسبوع") ||
    ...
    res
      .status(201)
      .json({ msg: "تم إرسال طلب الاشتراك بنجاح", order: newOrder });
  } catch (err) {
